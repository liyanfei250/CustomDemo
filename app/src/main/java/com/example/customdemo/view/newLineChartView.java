...
public class newLineChartView extends View {
    ...
    public newLineChartView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    public newLineChartView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //绘制Y轴
        canvas.drawLine(left, bottom, left, 0, linePaint);
        ...
        float min_weight = (weight - 50 * mScale) / (values.length);
        ...
        for (int i = 0; i < weeks.length; i++) {
            ...
            float x = (int) (50 * mScale + i * min_weight + min_weight / 2);
            canvas.drawText(weeks[i], x + min_weight / 4, y, linePaint);
        }
        ...
        for (int i = 0; i < values.length; i++) {
            int buttomR = (int) (getHeight() - th * 2);
            int topR = (int) ((max - values[i]) * unit + top);

            int leftR = (int) (50 * mScale + i * min_weight + min_weight / 2);
            int rightR = leftR + (int) (min_weight / 2);

            canvas.drawRect(new RectF(leftR, topR, rightR, buttomR), linePaint);

            canvas.drawText(values[i] + "", leftR + min_weight / 4, topR - 10 * mScale, linePaint);
        }
